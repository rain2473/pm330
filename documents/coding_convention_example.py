...
class Person(): # 클래스의 이름은 대문자로 시작, 가급적 한 음절로 구성, 부득이하게 두음절 이상으로 구성할 경우 Camel Notation(ex: MadGuy)으로 구성
    ...
    def __init__(self, prop1, prop2="abc"):  # 메서드의 이름은 동사로 시작, 각 음절 사이에는 언더바(_) 삽입, 함수 이름은 소문자로 구성, 함수의 리턴값은 타입만 기입
        """
...
def get_stock_name(ticker:str):  # 함수의 이름은 동사로 시작, 각 음절 사이에는 언더바(_) 삽입, 함수 이름은 소문자로 구성, 함수의 리턴값은 타입만 기입
    """
    <함수에 대한 개괄적인 설명> (~이다 체)
